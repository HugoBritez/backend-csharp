...
module.exports = function (dbInyectada) {
  ...
  async function getPedidosNuevo(
    ...
    try {
      // 1. Obtener pedidos base con filtros
      const pedidosBase = await obtenerPedidosBase(
        fecha_desde, fecha_hasta, nro_pedido, articulo, 
      ...
      const pedidosIds = pedidosBase.map(p => p.p_codigo);

      // 2. Obtener datos adicionales en paralelo
      const [
        detallesPedidos,
        cantidadesVendidas,
        preciosCompra,
        ...
        obtenerDetallesPedidos(pedidosIds),
        obtenerCantidadesVendidas(pedidosIds),
        obtenerPreciosCompra(pedidosIds, moneda),
        obtenerStockArticulos(pedidosIds),
        obtenerCantidadesPendientes(pedidosIds),
        obtenerAreasSecuencia(pedidosBase.map(p => p.p_area))
      ]);



      // 3. Combinar todos los datos
      return combinarDatosPedidos(
        pedidosBase,
        detallesPedidos,
        cantidadesVendidas,
        preciosCompra,
      ...
      console.error('Error en getPedidosNuevo:', error);
      throw error;
    }
  }

  // Función para obtener pedidos base
  async function obtenerPedidosBase(
    fecha_desde, fecha_hasta, nro_pedido, articulo,
    clientes, vendedores, sucursales, estado, moneda, factura
  ) {
    let where = "1 = 1";
  ...
  // Función para obtener detalles de pedidos
  async function obtenerDetallesPedidos(pedidosIds) {
    ...
    const query = `
      ...
      WHERE dp.dp_pedido IN (${pedidosIds.join(',')})
  ...
  // Función para obtener cantidades vendidas
  async function obtenerCantidadesVendidas(pedidosIds) {
    if (!pedidosIds || pedidosIds.length === 0) return [];
    ...
    const query = `
      SELECT
        ...
        dv.deve_articulo,
        dvv.loteid,
        SUM(dv.deve_cantidad) as cantidad_vendida,
        SUM(dv.deve_exentas) as exentas_vendidas,
        SUM(dv.deve_cinco) as cinco_vendidas,
        SUM(dv.deve_diez) as diez_vendidas
      ...
      WHERE v.ve_pedido IN (${pedidosIds.join(',')})
      AND v.ve_estado = 1
      GROUP BY v.ve_pedido, dv.deve_articulo, dvv.loteid
    `;

    return await db.sql(query);
  }

  // Función para obtener precios de compra
  async function obtenerPreciosCompra(pedidosIds, moneda) {
    if (!pedidosIds || pedidosIds.length === 0) return [];
    ...
    const query = `
      SELECT
        ...
        p.p_moneda,
        dc.dc_articulo,
        (dc.dc_precio + dc.dc_recargo) as precio_compra,
        c.co_fecha as fecha_compra
      FROM pedidos p
      INNER JOIN detalle_compras dc ON dc.dc_articulo IN (
        SELECT DISTINCT dp.dp_articulo 
        FROM detalle_pedido dp 
        WHERE dp.dp_pedido = p.p_codigo
      )
      INNER JOIN compras c ON dc.dc_compra = c.co_codigo
      WHERE p.p_codigo IN (${pedidosIds.join(',')})
      AND c.co_estado = 1
      AND c.co_fecha <= p.p_fecha
      ${moneda ? `AND c.co_moneda = ${moneda}` : ''}
      ORDER BY p.p_codigo, dc.dc_articulo, c.co_fecha DESC
    `;

    return await db.sql(query);
  }

  // Función para obtener stock de artículos
  async function obtenerStockArticulos(pedidosIds) {
    if (!pedidosIds || pedidosIds.length === 0) return [];
  ...
  // Función para obtener cantidades pendientes
  async function obtenerCantidadesPendientes(pedidosIds) {
    if (!pedidosIds || pedidosIds.length === 0) return [];
    ...
    const query = `
      SELECT
        dp.dp_articulo,
        dp.dp_codigolote,
        SUM(dp.dp_cantidad) as cant_pendiente
      FROM detalle_pedido dp
      INNER JOIN pedidos p ON dp.dp_pedido = p.p_codigo
      WHERE p.p_estado = 1
      AND dp.dp_articulo IN (
        SELECT DISTINCT dp2.dp_articulo 
        FROM detalle_pedido dp2 
        WHERE dp2.dp_pedido IN (${pedidosIds.join(',')})
      )
      GROUP BY dp.dp_articulo, dp.dp_codigolote
    `;

    return await db.sql(query);
  }

  // Función para obtener áreas de secuencia
  async function obtenerAreasSecuencia(areasIds) {
    if (!areasIds || areasIds.length === 0) return [];
    
    const areasUnicas = [...new Set(areasIds)];
    const query = `
      SELECT
        arse.ac_area,
        area.a_descripcion as siguiente_area
      FROM area_secuencia arse
      INNER JOIN area area ON arse.ac_secuencia_area = area.a_codigo
      WHERE arse.ac_area IN (${areasUnicas.join(',')})
      ORDER BY arse.ac_codigo
    `;
  ...
  function combinarDatosPedidos(
    ...
    const detallesPorPedido = agruparPor(detallesPedidos, 'dp_pedido');
    const ventasPorPedido = agruparPor(cantidadesVendidas, 've_pedido');
    const preciosPorPedido = agruparPor(preciosCompra, 'pedido_id');
    const stockPorPedido = agruparPor(stockArticulos, 'pedido_id');
    const pendientesPorArticulo = agruparPor(cantidadesPendientes, 'dp_articulo');
    const areasSecuenciaMap = new Map(areasSecuencia.map(a => [a.ac_area, a.siguiente_area]));

    return pedidosBase.map(pedido => {
      const detalles = detallesPorPedido.get(pedido.p_codigo) || [];
      const ventas = ventasPorPedido.get(pedido.p_codigo) || [];
      const precios = preciosPorPedido.get(pedido.p_codigo) || [];
      const stock = stockPorPedido.get(pedido.p_codigo) || [];

      const detallesEnriquecidos = detalles.map(detalle => {
        const venta = ventas.find(v => 
          v.deve_articulo === detalle.dp_articulo && 
          v.loteid === detalle.dp_codigolote
        );
        
        const precioCompra = precios.find(p => 
          p.dc_articulo === detalle.dp_articulo
        );
        
        const stockItem = stock.find(s => 
          s.dp_articulo === detalle.dp_articulo && 
          s.dp_codigolote === detalle.dp_codigolote
        );
        
        const pendiente = pendientesPorArticulo.get(detalle.dp_articulo) || [];
        const cantPendiente = pendiente.find(p => 
          p.dp_codigolote === detalle.dp_codigolote
        );

        const cantidadVendida = venta ? venta.cantidad_vendida : 0;
        const precioCompraValor = precioCompra ? precioCompra.precio_compra : detalle.ar_pcg;
        const cantStock = stockItem ? stockItem.cant_stock : 0;
        const cantPendienteValor = cantPendiente ? cantPendiente.cant_pendiente : 0;

        return {
          codigo: detalle.dp_articulo,
          descripcion_articulo: detalle.dp_descripcion_art || detalle.ar_descripcion,
          cantidad_vendida: detalle.dp_cantidad - cantidadVendida,
          bonificacion: detalle.dp_bonif === 0 ? 'V' : 'B',
          d_cantidad: detalle.d_cantidad,
          precio: formatNumber(detalle.dp_precio),
          ultimo_precio: formatNumber(precioCompraValor),
          porc_costo: calcularPorcentajeCosto(detalle.dp_precio, precioCompraValor),
          porcentaje: calcularPorcentaje(detalle.dp_precio, precioCompraValor),
          descuento: formatNumber(detalle.dp_descuento),
          exentas: formatNumber(detalle.dp_exentas - (venta ? venta.exentas_vendidas : 0)),
          cinco: formatNumber(detalle.dp_cinco - (venta ? venta.cinco_vendidas : 0)),
          diez: formatNumber(detalle.dp_diez - (venta ? venta.diez_vendidas : 0)),
          dp_lote: detalle.dp_lote || '',
          vencimiento: detalle.dp_vence === '0001-01-01' ? '' : formatDate(detalle.dp_vence),
          comision: detalle.dp_porcomision === 0 ? detalle.dp_porcomision : detalle.ar_comision,
          actorizado: detalle.dp_actorizado,
          obs: detalle.dp_obs || '',
          cant_stock: cantStock,
          dp_codigolote: detalle.dp_codigolote,
          cant_pendiente: cantPendienteValor,
          cantidad_verificada: detalle.dp_cantidad === (detalle.dp_cantidad - cantidadVendida) ? 1 : 0
        };
      });

      const total = detalles.reduce((sum, d) => 
        sum + (d.dp_cantidad * (d.dp_precio - d.dp_descuento)), 0
      );

      return {
        pedido_id: pedido.p_codigo,
        cliente: pedido.cliente,
        moneda: pedido.moneda,
        fecha: pedido.fecha,
        factura: pedido.factura,
        area: pedido.area,
        siguiente_area: areasSecuenciaMap.get(pedido.p_area) || '-',
        estado: pedido.estado,
        estado_num: pedido.estado_num,
        ...
        imprimir: pedido.imprimir,
        imprimir_preparacion: pedido.imprimir_preparacion,
        cliente_id: pedido.cliente_id,
        cantidad_cajas: pedido.cantidad_cajas,
        obs: pedido.obs,
        total: formatNumber(total),
        acuerdo: pedido.p_acuerdo === 1 ? 'Tiene acuerdo comercial' : '',
  ...
  function calcularPorcentajeCosto(precioVenta, precioCompra) {
    if (!precioCompra || precioCompra === 0) return 0;
    return Math.round(((precioVenta - precioCompra) * 100) / precioCompra * 100) / 100;
  }

  function calcularPorcentaje(precioVenta, precioCompra) {
    if (!precioVenta || precioVenta === 0) return 0;
    return Math.round(((precioVenta - precioCompra) * 100) / precioVenta * 100) / 100;
  }
      ...
      for (const item of detalle_pedido) {
        await db.agregar(
          DETALLES,
          {
            ...
            dp_descripcion_art: item.dp_descripcion_art,
          },
          0,
          "dp_codigo"
        );
