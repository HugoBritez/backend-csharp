...
module.exports = function (dbInyectada) {
  ...
  function todos(
    ...
    let having = "";
    let order = "";
    if (fecha_desde != "") where += ` AND a.a_fecha >= '${fecha_desde}'`;
    if (fecha_hasta != "") where += ` AND a.a_fecha <= '${fecha_hasta}'`;
    if (cliente > 0 || cliente != null) where += ` AND a.a_cliente IN  (${cliente})`;
    if (vendedor > 0 && vendedor != null) where += ` AND a.a_vendedor IN (${vendedor})`;
    if (visitado != -1) where += ` AND a.a_visitado = ${visitado}`;
    if (estado != -1) where += ` AND a.a_estado = ${estado}`;
    if (planificacion != -1) where += ` AND a.a_planificacion = ${planificacion}`;
    if (notas == 0) having += " HAVING cant_notas = 0";
    if (notas == 1) having += " HAVING cant_notas > 0";

    switch (orden) {
      case "0": //Prioridad
        order = "ORDER BY a.a_prioridad DESC, a.a_fecha ASC";
        break;
      case "1": //Código
        order = "ORDER BY a.a_codigo ASC";
        break;
      case "2": //Fecha
        order = "ORDER BY a.a_fecha DESC, a.a_hora ASC";
        break;
    }
    ...
    let query = `SELECT
                    ...
                    IF(a.a_prioridad=0, "Común", IF(a.a_prioridad=1, "Baja", IF(a.a_prioridad=2, "Moderada", IF(a.a_prioridad=3, "Alta", "-")))) AS prioridad,
  ...
  async function agregarLocalizacion(datos) {
    const maxRetries = 3;
