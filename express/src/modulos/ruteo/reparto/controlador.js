module.exports = function (dbInyectada) {
  ...
  `;
    ...
    console.log('Detalles completos:', JSON.stringify(detalles, null, 2));
    return result;
  }
  ...
  async function listarRutas(query) {
    ...
    const querySql = `
      ...
      LEFT JOIN monedas m ON m.mo_codigo = r.r_moneda
      WHERE r.r_estado = 1
      AND r.r_fecha BETWEEN DATE_SUB('${fecha}', INTERVAL 7 DAY) AND '${fecha}'
      ${whereVendedor}
    `;
  ...
  async function enviarMensajeTelegramAsync(chatId, mensaje, latitud, longitud) {
    try {
      console.log(`[${new Date().toISOString()}] Iniciando envío de mensaje a ${chatId}`);
      await telegramBot.enviarMensajeConUbicacion(chatId, mensaje, latitud, longitud);
