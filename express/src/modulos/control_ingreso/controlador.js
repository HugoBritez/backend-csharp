  ...
  async function getFacturas(
    ...
    if (nro_factura) {
      ...
      if (!fecha_desde && !fecha_hasta) {
        ...
        where += ` AND co.co_fecha BETWEEN '${fecha_desde}' AND '${fecha_hasta}'`;
      } else if (fecha_desde && !fecha_hasta) {
        where += ` AND co.co_fecha >= '${fecha_desde}'`;
      } else if (!fecha_desde && fecha_hasta) {
        where += ` AND co.co_fecha <= '${fecha_hasta}'`;
      }
    ...
    const query = `
        ...
        co.co_proveedor  as nro_proveedor,
        pro.pro_razon as proveedor,
        pro.pro_codigo as proveedor_id,
        co.co_verificado as verificado,
        co_responsable_ubicacion as responsable_ubicacion,
        case
        ...
        LEFT JOIN proveedores pro ON co.co_proveedor = pro.pro_codigo
  ...
  async function reporteIngresos(
    ...
    const query = `
        SELECT
          ...
          co.co_proveedor as proveedor_id,
          co.co_verificado as verificado,
        ...
        FROM compras co
        INNER JOIN depositos dep ON co.co_deposito = dep.dep_codigo
        INNER JOIN proveedores pr ON co.co_proveedor = pr.pro_codigo
