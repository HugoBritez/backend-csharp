...
router.get("/listar", seguridad(), listarArticulos)
...
async function listarArticulos(req, res, next){
  try {
    const { busqueda, estado, marca, categoria, proveedor } = req.query;
    const items = await controlador.listarArticulos(busqueda, estado, marca, categoria, proveedor);
