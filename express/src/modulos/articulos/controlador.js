...
module.exports = function (dbInyectada) {
  ...
  const consultaArticulosSimplificado = async (
    ...
    // Construir WHERE clause con índices optimizados
    if (articulo_id) {
      where += ` AND ar.ar_codigo = '${articulo_id}'`;
    } else if (cod_interno) {
      where += ` AND ar.ar_cod_interno = '${cod_interno}'`;
    } else {
      if (codigo_barra) {
        where += ` AND (ar.ar_codbarra = '${codigo_barra}' OR al.al_codbarra = '${codigo_barra}')`;
      } else if (busqueda) {
        const filtrosEspecificos = [marca, categoria, ubicacion, proveedor].filter(Boolean);

        if (filtrosEspecificos.length > 0) {
          // JOINs condicionales solo cuando se necesiten
          let condicionesFiltros = [];

          if (marca) {
            if (!joins.includes('marcas')) {
              joins += " LEFT JOIN marcas ma ON ar.ar_marca = ma.ma_codigo";
            }
            condicionesFiltros.push(`ma.ma_descripcion LIKE '%${busqueda}%'`);
          }
          if (categoria) {
            if (!joins.includes('subcategorias')) {
              joins += " LEFT JOIN subcategorias sc ON ar.ar_subcategoria = sc.sc_codigo";
              joins += " LEFT JOIN categorias ca ON sc.sc_categoria = ca.ca_codigo";
            }
            condicionesFiltros.push(`ca.ca_descripcion LIKE '%${busqueda}%'`);
          }
          if (ubicacion) {
            if (!joins.includes('ubicaciones')) {
              joins += " LEFT JOIN ubicaciones ub ON ar.ar_ubicacicion = ub.ub_codigo";
            }
            condicionesFiltros.push(`ub.ub_descripcion LIKE '%${busqueda}%'`);
          }
          if (proveedor) {
            condicionesFiltros.push(`EXISTS (
              SELECT 1 FROM articulos_proveedores ap 
              INNER JOIN proveedores p ON ap.arprove_prove = p.pro_codigo
              WHERE ap.arprove_articulo = ar.ar_codigo 
          ...
          const condiciones = palabras.map(
            (palabra) =>
              `(ar.ar_descripcion LIKE '%${palabra}%' OR ar.ar_codbarra = '${palabra}' OR al.al_codbarra = '${palabra}')`
          );
          where += ` AND (${condiciones.join(" AND ")})`;
        }
      }
    ...
    // Configurar precios según moneda
    if (moneda == 1 || moneda == "1" || moneda == null || moneda == undefined) {
      ...
      moneda_query = `
        ...
        null as precio_credito,
        null as precio_mostrador,
        null as precio_4,
      `;
    ...
    // Query principal optimizada - SIN subconsultas complejas
    const query = `
      SELECT
         ar.ar_codigo as id_articulo,
         ar.ar_codbarra as codigo_barra,
         ...
         ${moneda_query}
         COALESCE(ub.ub_descripcion, '') as ubicacion,
         COALESCE(s.s_descripcion, '') as sub_ubicacion,
         COALESCE(ma.ma_descripcion, '') as marca,
         COALESCE(sc.sc_descripcion, '') as subcategoria,
         COALESCE(ca.ca_descripcion, '') as categoria,
         ar.ar_iva as iva,
         ar.ar_vencimiento as vencimiento_validacion,
         COALESCE(iva.iva_descripcion, '') as iva_descripcion,
         COALESCE(ta.t_descripcion, '') as talle,
         COALESCE(co.c_descripcion, '') as color,
         ar.ar_editar_desc as editar_nombre,
         
         -- Estado de vencimiento específico del lote
         CASE
           WHEN al.al_vencimiento = '0001-01-01' OR al.al_vencimiento IS NULL THEN 'SIN_VENCIMIENTO'
           WHEN DATEDIFF(al.al_vencimiento, CURDATE()) < 0 THEN 'VENCIDO'
           WHEN DATEDIFF(al.al_vencimiento, CURDATE()) <= 120 THEN 'PROXIMO'
           ELSE 'VIGENTE'
         END as estado_vencimiento
      ...
      LEFT JOIN colores co ON al.al_color = co.c_codigo
      LEFT JOIN ubicaciones ub ON ar.ar_ubicacicion = ub.ub_codigo
      LEFT JOIN sub_ubicacion s ON ar.ar_sububicacion = s.s_codigo
      ${!joins.includes('marcas') ? 'LEFT JOIN marcas ma ON ar.ar_marca = ma.ma_codigo' : ''}
      ${!joins.includes('subcategorias') ? 'LEFT JOIN subcategorias sc ON ar.ar_subcategoria = sc.sc_codigo' : ''}
      ${!joins.includes('categorias') ? 'LEFT JOIN categorias ca ON sc.sc_categoria = ca.ca_codigo' : ''}
      LEFT JOIN iva ON ar.ar_iva = iva.iva_codigo
      ${joins}
      
      WHERE ar.ar_estado = 1
      ${where}
      ORDER BY ar.ar_descripcion, al.al_lote
      LIMIT 30
    `;

    console.log(query);
    const articulos = await db.sql(query);

    // Enriquecer con datos adicionales solo si es necesario
    if (articulos.length > 0) {
      const articulosEnriquecidos = await Promise.all(
        articulos.map(async (articulo) => {
          const [fechaUltimaVenta, proveedor, stockTotal, lotes, depositos] = await Promise.all([
            obtenerFechaUltimaVenta(articulo.id_articulo),
            obtenerProveedor(articulo.id_articulo),
            obtenerStockTotal(articulo.id_articulo),
            obtenerLotes(articulo.id_articulo),
            obtenerStockPorDeposito(articulo.id_articulo)
          ]);
      ...
      return articulosEnriquecidos;
    }

    return articulos;
  };

  // Funciones auxiliares para obtener datos adicionales
  const obtenerFechaUltimaVenta = async (articuloId) => {
    const query = `
      SELECT DATE_FORMAT(MAX(ve.ve_fecha), '%d/%m/%Y') as fecha
      FROM ventas ve
      INNER JOIN detalle_ventas dv ON ve.ve_codigo = dv.deve_venta
      WHERE dv.deve_articulo = ${articuloId}
    `;
    const result = await db.sql(query);
    return result[0]?.fecha || null;
  };

  const obtenerProveedor = async (articuloId) => {
    const query = `
      SELECT GROUP_CONCAT(DISTINCT p.pro_razon SEPARATOR ', ') as proveedor
      FROM articulos_proveedores ap 
      INNER JOIN proveedores p ON ap.arprove_prove = p.pro_codigo
      WHERE ap.arprove_articulo = ${articuloId}
    `;
    const result = await db.sql(query);
    return result[0]?.proveedor || '';
  };

  const obtenerStockTotal = async (articuloId) => {
    const query = `
      SELECT CAST(COALESCE(SUM(al.al_cantidad), 0) AS SIGNED) as stock_total
      FROM articulos_lotes al
      WHERE al.al_articulo = ${articuloId}
    `;
    const result = await db.sql(query);
    return result[0]?.stock_total || 0;
  };

  const obtenerLotes = async (articuloId) => {
    const query = `
      SELECT 
        al.al_codigo as id,
        al.al_lote as lote,
        CAST(al.al_cantidad AS SIGNED) as cantidad,
  ...
  const obtenerStockPorDeposito = async (articuloId) => {
    const query = `
      SELECT 
        dep.dep_codigo as codigo,
        dep.dep_descripcion as descripcion,
        CAST(SUM(al.al_cantidad) AS SIGNED) as stock
      FROM articulos_lotes al
      INNER JOIN depositos dep ON al.al_deposito = dep.dep_codigo
      WHERE al.al_articulo = ${articuloId}
      GROUP BY al.al_deposito, dep.dep_codigo, dep.dep_descripcion
    `;
    return await db.sql(query);
  };
...
`;
  ...
  async function todosNuevo(
    ...
    `;
      ...
      SELECT DISTINCT
        ...
        (case when ${moneda || 1} = 1 then 'Gs' when ${moneda || 1
        } = 2 then 'USD' when ${moneda || 1} = 3 then 'BRL' when ${moneda || 1
            ...
            FROM
                ...
                LEFT JOIN (SELECT
                            ...
                            FROM
                                ...
                                INNER JOIN compras ON detalle_ventas.deve_venta = ventas.ve_codigo
  ...
  async function insertarInventario(consulta) {
    ...
    const inventarioQuery = `
        INSERT INTO inventarios (fecha, hora, operador, sucursal, deposito, tipo, estado, in_obs, nro_inventario) 
        VALUES ('${inventario.fecha}', '${inventario.hora}', ${inventario.operador
      }, ${inventario.sucursal}, 
                ${inventario.deposito}, ${inventario.tipo}, ${inventario.estado
      }, '${inventario.in_obs}', ${!inventario.nro_inventario ? 1 : inventario.nro_inventario
      })`;
...
`;
  ...
  const listarArticulos = async (busqueda, estado, marca, categoria, proveedor) => {
    let where = " 1=1";

    console.log("busqueda", busqueda, typeof busqueda);
    console.log("estado", estado, typeof estado);
    console.log("marca", marca, typeof marca);
    console.log("categoria", categoria, typeof categoria);
    console.log("proveedor", proveedor, typeof proveedor);

    if (estado != null) {
      where += ` AND a.ar_estado = ${estado}`;
    }

    if (busqueda && busqueda.length > 0) {
      // Convertir strings booleanos a booleanos reales
      const marcaBool = marca === 'true' || marca === true;
      const categoriaBool = categoria === 'true' || categoria === true;
      const proveedorBool = proveedor === 'true' || proveedor === true;

      if (marcaBool) {
        where += ` AND m.ma_descripcion LIKE '%${busqueda}%'`;
      }
      else if (categoriaBool) {
        where += ` AND ca.ca_descripcion LIKE '%${busqueda}%'`;
      }
      else if (proveedorBool) {
        where += ` AND p.pro_razon LIKE '%${busqueda}%'`;
      }
      else {
        const palabras = busqueda.split(" ").filter((p) => p.length > 0);
        const condiciones = palabras.map(
          (palabra) =>
            `(a.ar_descripcion LIKE '%${palabra}%' OR a.ar_codbarra = '${palabra}' OR a.ar_cod_interno = '${palabra}' )`
        );
        where += ` AND (${condiciones.join(" AND ")})`;
      }
    }
    ...
    const query =
      `
      SELECT
	      a.ar_cod_interno AS codRef,
	      ifnull(a.ar_codbarra,"") as codigoBarra,
	      a.ar_descripcion as descripcion,
	      CAST(ifnull(a.ar_cod_acri,'') as char) As codAcri,
	       ifnull((Select
		    	    e.pro_razon
		    	From
		    	    articulos_proveedores po Inner Join
		    	    proveedores e On po.arprove_prove = e.pro_codigo
		    	Where
		    	    po.arprove_articulo = a.ar_codigo
		    	order by po.arprove_codigo desc limit 1),'') as proveedor,
	      ifnull(m.ma_descripcion, '') as marca,
	      IFNULL(v.iva_descripcion, "") AS iva,
	      IFNULL(ca.ca_descripcion,'') AS categoria,
	      a.ar_codigo as codigo,
	      IFNULL(cm.c_descripcion,'') as tipoMedida,
	      IFNULL(um.um_descripcion,'') as unidadMedida,
	      IF(a.ar_fraccionamiento = 1, 'No', 'Si') as fraccionable,
	      a.ar_estado as estado
	    FROM
	    articulos AS a
	    INNER JOIN subcategorias sc ON a.ar_subcategoria = sc.sc_codigo
	    INNER JOIN categorias ca ON sc.sc_categoria = ca.ca_codigo
	    INNER JOIN marcas m ON a.ar_marca = m.ma_codigo
	    INNER JOIN iva v ON a.ar_iva = v.iva_codigo
	    INNER JOIN unidadmedidas um ON a.ar_unidadmedida = um.um_codigo
      INNER JOIN control_medicamento cm ON a.ar_tipo_control = cm.c_codigo
      WHERE ${where}
      ORDER BY a.ar_descripcion ASC;
    `
