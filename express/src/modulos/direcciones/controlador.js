...
module.exports = function (dbInyectada) {
    ...
    const query = `
        SELECT 
          ...
          CONCAT(
            ...
            LPAD(d.d_predio, 3, '0'), '-',
            LPAD(d.d_piso, 3, '0'), '-',
            LPAD(d.d_direccion, 3, '0')
          ) as direccion_completa
        FROM direcciones d ${where}
    `;
  ...
  async function generarSecuenciaDirecciones(rango) {
    ...
    // Convertir a números y generar secuencia numérica para las calles
    const calleInicial = parseInt(d_calle_inicial);
    const calleFinal = parseInt(d_calle_final || d_calle_inicial);

    for (let calle = calleInicial; calle <= calleFinal; calle++) {
      // Formatear calle con ceros a la izquierda (ej: 001, 002, 010, 100, etc.)
      const calleFormateada = calle.toString().padStart(3, '0');
      
      // Si predio_final es null, usar solo predio_inicial (o 0 si también es null)
      const predioInicial = d_predio_inicial || 0;
      const predioFinal = d_predio_final || predioInicial;
      for (let predio = predioInicial; predio <= predioFinal; predio++) {
        // Predio se mantiene como número
        
        // Si piso_final es null, usar solo piso_inicial (o 0 si también es null)
        const pisoInicial = d_piso_inicial || 0;
        const pisoFinal = d_piso_final || pisoInicial;
        for (let piso = pisoInicial; piso <= pisoFinal; piso++) {
          // Piso se mantiene como número
          
          // Si direccion_final es null, usar solo direccion_inicial (o 0 si también es null)
          const direccionInicial = d_direccion_inicial || 0;
          const direccionFinal = d_direccion_final || direccionInicial;
          for (let dir = direccionInicial; dir <= direccionFinal; dir++) {
            // Direccion se mantiene como número
            
            secuencia.push({
              calle: calleFormateada,  // String formateado
              predio: predio,          // Número
              piso: piso,              // Número
              direccion: dir,          // Número
            });
          }
        }
      }
  ...
  // Función auxiliar para formatear direcciones en la presentación
  function formatearDireccion(calle, predio, piso, direccion) {
    const calleFormateada = calle.toString().padStart(3, '0');
    const predioFormateado = predio.toString().padStart(3, '0');
    const pisoFormateado = piso.toString().padStart(3, '0');
    const direccionFormateada = direccion.toString().padStart(3, '0');
    
    return `${calleFormateada}-${predioFormateado}-${pisoFormateado}-${direccionFormateada}`;
  }
  ...
  async function crearDireccionesEnSecuencia(direccion) {
    ...
    for (const item of secuencia) {
      const nuevaDireccion = {
        d_calle: item.calle,           // Ya es string formateado
        d_predio: item.predio,         // Ya es número
        d_piso: item.piso,             // Ya es número
        d_direccion: item.direccion,   // Ya es número
