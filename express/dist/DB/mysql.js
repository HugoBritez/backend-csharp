...
function todos(tabla, campos, where) {
    ...
    return new Promise((resolve, reject) => {
        conexion.query(`SELECT ${campos} FROM ${tabla} WHERE ${where}  `, (error, result) => {
            if (error)
                return reject(error);
            resolve(result);
        });
...
function uno(tabla, primary_key, campos) {
    return new Promise((resolve, reject) => {
        conexion.query(`SELECT ${campos} FROM ${tabla} WHERE ${primary_key} `, (error, result) => {
            if (error)
                return reject(error);
            resolve(result);
        });
...
function insertar(tabla, data) {
    return new Promise((resolve, reject) => {
        conexion.query(`INSERT INTO ${tabla} SET ?`, data, (error, result, filed) => {
            if (error)
                return reject(error);
            resolve(result);
        });
...
function actualizar(tabla, data, primary_key_value, primary_key_name) {
    return new Promise((resolve, reject) => {
        conexion.query(`UPDATE ${tabla} SET ? WHERE ${primary_key_name} = ${primary_key_value} `, [data], (error, result) => {
            if (error)
                return reject(error);
            resolve(result);
        });
...
function eliminar(tabla, where_update, set_campo) {
    return new Promise((resolve, reject) => {
        conexion.query(` UPDATE ${tabla} SET ${set_campo} WHERE ${where_update} `, (error, result) => {
            if (error)
                return reject(error);
            resolve(result);
        });
...
function login(tabla, user, pass) {
