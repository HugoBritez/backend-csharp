...
function atender(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.atender(req.query.codigo, req.query.box);
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
...
function descartar(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.descartar(req.query.codigo);
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
...
function modificar(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.modificar(req.query.codigo, req.query.campo, req.query.valor);
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
...
function listar(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        AND p.preag_sucursal = ${req.query.suc}` + tipo + etapa + especialidad;
            let items = yield controlador.sql(sqlquery);
            for (registro in items) {
                //Recuperamos nombre, sea paciente o contribuyente
                let nombre_buscado = '';
                let actual = items[registro];
                if (actual.preag_paciente != 0) {
                    const paciente = yield controlador.traer_paciente(actual.preag_paciente);
                    if (paciente.length > 0) {
                        nombre_buscado = paciente[0].nombre;
                    }
                }
                else {
                    const paciente = yield controlador.buscar_paciente(actual.preag_documento);
                    if (paciente.length > 0) {
                        nombre_buscado = paciente[0].nombre;
                        controlador.modificar(actual.preag_codigo, `preag_paciente`, paciente[0].pac_codigo);
                    }
                    else {
                        const contribuyente = yield controlador.buscar_contribuyente(actual.preag_documento);
...
function buscar(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const paciente = yield controlador.buscar_paciente(req.query.documento);
            let persona = {
                per_pac_cod: 0,
                per_nombre: ''
            };
            if (paciente.length === 0) { //No encontró paciente, debemos buscar entre contribuyentes
                const contribuyente = yield controlador.buscar_contribuyente(req.query.documento);
...
function agregar(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const ultimo = yield controlador.get_ultimo(req.body.tipo_age_cod, datos.preag_fecha);
            ...
            const agregado = yield controlador.agregar(datos); //Devuelve registro nuevo
            ...
            if (datos.preag_paciente > 0) {
                const resp_seg = yield controlador.sql(`SELECT s.se_descripcion FROM pacientes p LEFT JOIN seguros s ON p.pac_seguro = s.se_codigo WHERE p.pac_codigo = ${datos.preag_paciente}`);
...
function cargar_cola(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.cargar_cola(req.body);
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
...
function buscar_cola(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const item = yield controlador.buscar_cola();
            if (item.length > 0) {
                //Si no tiene nombre, buscamos de nuevo a ver si ya se cargó ficha de paciente
                if (!item[0].nombre) {
                    let paciente = yield controlador.buscar_paciente(`'${item[0].documento}'`);
                    if (paciente.length > 0) {
                ...
                yield controlador.eliminar_cola(item[0].preag_id, item[0].sucursal);
...
function esp_preag(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.esp_preag();
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
