...
const TABLA = "articulos_lotes al INNER JOIN articulos a ON al.al_articulo = a.ar_codigo INNER JOIN depositos de ON al.al_deposito = de.dep_codigo INNER JOIN unidadmedidas um ON a.ar_unidadmedida = um.um_codigo INNER JOIN marcas ma ON a.ar_marca = ma.ma_codigo LEFT JOIN subcategorias sc ON a.ar_subcategoria= sc.sc_codigo LEFT JOIN articulos_proveedores ap ON a.ar_codigo = ap.arprove_articulo LEFT JOIN proveedores p ON ap.arprove_codigo = p.pro_codigo LEFT JOIN ubicaciones ub ON a.ar_ubicacicion = ub.ub_codigo";
...
module.exports = function (dbInyectada) {
    ...
    const buscarArticulos = (articulo_id_1, busqueda_1, codigo_barra_1, ...args_1) => __awaiter(this, [articulo_id_1, busqueda_1, codigo_barra_1, ...args_1], void 0, function* (articulo_id, busqueda, codigo_barra, moneda = 1, stock, deposito, marca, categoria, ubicacion, proveedor, cod_interno, lote, negativo) {
    ...
    const consultaArticulosSimplificado = (articulo_id_1, busqueda_1, codigo_barra_1, ...args_1) => __awaiter(this, [articulo_id_1, busqueda_1, codigo_barra_1, ...args_1], void 0, function* (articulo_id, busqueda, codigo_barra, moneda = 1, stock, deposito, marca, categoria, ubicacion, proveedor, cod_interno) {
        console.log("articulo_id", articulo_id);
        console.log("codigo_barra", codigo_barra);
        console.log("busqueda", busqueda);
        console.log("moneda", moneda);
    ...
    const reporteDeAnomalias = (nro_inventario, sucursal, deposito) => __awaiter(this, void 0, void 0, function* () {
        const query = `
          SELECT DISTINCT
    ...
    const scannearItemInventarioAuxiliar = (id_articulo_1, id_lote_1, cantidad_1, ...args_1) => __awaiter(this, [id_articulo_1, id_lote_1, cantidad_1, ...args_1], void 0, function* (id_articulo, id_lote, cantidad, lote = "", codigo_barras = "", id_inventario) {
        try {
            ...
            if (codigo_barras && codigo_barras.trim().length > 0) {
                ...
                yield db.sql(queryCodigos2); // solo para gaesa y compañia
            }
        ...
        WHERE id_articulo = ${id_articulo} AND id_lote = ${id_lote} AND id_inventario = ${id_inventario}`;
            ...
            return yield db.sql(queryScanner);
        }
        catch (error) {
    ...
    const mostrarItemsDelInventarioAuxiliar = (id, id_inventario, buscar, deposito) => __awaiter(this, void 0, void 0, function* () {
        let where = "";
        if (buscar) {
            const palabras = buscar.split(" ").filter((p) => p.length > 0);
            const condiciones = palabras.map((palabra) => `(ar.ar_descripcion LIKE '%${palabra}%' OR al.al_codbarra = '${palabra}' OR al.al_lote = '${palabra}' OR al.al_codigo = '${palabra}' OR ar.ar_cod_interno = '${palabra}' OR ar.ar_codbarra = '${palabra}')`);
            where += ` AND (${condiciones.join(" AND ")})`;
        }
        if (deposito) {
    ...
    const mostrarItemsDelInventarioAuxiliarPrincipal = (id_1, ...args_1) => __awaiter(this, [id_1, ...args_1], void 0, function* (id, scanneado = false, deposito, sucursal, buscar, id_inventario) {
        console.log(scanneado);
        let scanneado_query = "";
        let scanneado_query_stock = "CAST(inventario_auxiliar_items.cantidad_inicial AS SIGNED) as stock";
        ...
        if (buscar) {
            const palabras = buscar.split(" ").filter((p) => p.length > 0);
            const condiciones = palabras.map((palabra) => `(ar.ar_descripcion LIKE '%${palabra}%' OR al.al_codbarra = '${palabra}' OR al.al_lote = '${palabra}' OR al.al_codigo = '${palabra}' OR ar.ar_cod_interno = '${palabra}')`);
            where += ` AND (${condiciones.join(" AND ")})`;
        }
        if (id_inventario) {
            where += ` AND inventario_auxiliar_items.id_inventario = ${id_inventario}`;
  ...
  `;
    ...
    function cerrarInventarioAuxiliar(id, operador, sucursal, deposito, nro_inventario, autorizado) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield db.sql(updateInventarioQuery);
     ...
     `;
                    yield db.sql(agregarRegistroInventarioQuery);
                    const autorizarInventarioAuxiliarQuery = `UPDATE inventario_auxiliar SET autorizado = 1 WHERE id = ${id}`;
                    yield db.sql(autorizarInventarioAuxiliarQuery);
                    console.log("agregando registro inventarios", agregarRegistroInventarioQuery);
                }
                yield db.sql("COMMIT");
                return true;
            }
    ...
    function ultimoInventarioAuxiliar(deposito, sucursal, nro_inventario) {
        return __awaiter(this, void 0, void 0, function* () {
...
`;
    ...
    ORDER BY id DESC LIMIT 1`;
            console.log(query);
            console.log(yield db.sql(query));
            const result = yield db.sql(query);
            return result[0];
        });
    }
    function insertarInventarioAuxiliar(inventario) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
        ...
        VALUES (NOW(), NOW(), ${operador}, ${sucursal}, ${deposito}, ${estado}, '${obs}', ${nro_inventario})`;
                const inventarioResult = yield db.sql(inventarioQuery);
    ...
    function insertarInventarioAuxiliarItems(inventario_items, inventario_id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const todosLosArticulos = inventario_items
                    ...
                    .join(",");
                const existenArticulos = yield db.sql(`
    ...
    function insertarConteoScanner(cantidad, id_articulo, id_lote, id_inventario) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `
    ...
    function itemTomaInventarioScanner(deposito_id, articulo_id, ubicacion, sub_ubicacion, categorias) {
        return __awaiter(this, void 0, void 0, function* () {
            let where = `WHERE 1=1`;
            if (deposito_id)
                where += ` AND al.al_deposito = ${deposito_id}`;
            if (articulo_id)
                where += ` AND al.al_articulo = ${articulo_id}`;
            if (ubicacion)
                where += ` AND ar.ar_ubicacicion = ${ubicacion}`;
            if (sub_ubicacion)
                where += ` AND ar.ar_sububicacion = ${sub_ubicacion}`;
            if (categorias)
                where += ` AND ca.ca_codigo IN (${categorias})`;
            const query = `
    SELECT
    ub.ub_descripcion as ubicacion,
    s.s_descripcion as sub_ubicacion,
    ...
    function itemTomaInventario(deposito_id, articulo_id, ubicacion, sub_ubicacion, categorias, marcas, secciones) {
        return __awaiter(this, void 0, void 0, function* () {
            let where = `WHERE 1=1`;
            if (deposito_id)
                where += ` AND al.al_deposito = ${deposito_id}`;
            if (articulo_id)
                where += ` AND al.al_codigo = ${articulo_id}`;
            if (ubicacion)
                where += ` AND ar.ar_ubicacicion = ${ubicacion}`;
            if (sub_ubicacion)
                where += ` AND ar.ar_sububicacion = ${sub_ubicacion}`;
            if (categorias)
                where += ` AND ca.ca_codigo IN (${categorias})`;
            if (marcas)
                where += ` AND ar.ar_marca IN (${marcas})`;
            if (secciones)
                where += ` AND ar.ar_seccion IN (${secciones})`;
            const query = `
    SELECT
    ub.ub_descripcion as ubicacion,
    s.s_descripcion as sub_ubicacion,
    ...
    function todosNuevo(busqueda_1, deposito_1, stock_1, marca_1, categoria_1, subcategoria_1, proveedor_1, ubicacion_1, servicio_1, moneda_1, unidadMedida_1) {
        return __awaiter(this, arguments, void 0, function* (busqueda, deposito, stock, marca, categoria, subcategoria, proveedor, ubicacion, servicio, moneda, unidadMedida, pagina = 1, limite = 50, tipoValorizacionCosto) {
            try {
                pagina = parseInt(pagina) || 1;
                limite = parseInt(limite) || 50;
                console.log("Página:", pagina, "Límite:", limite, "Offset calculado:", (pagina - 1) * limite);
                let where = "WHERE 1=1";
                let ultimocostocompra = "";
                if (busqueda) {
                    const palabras = busqueda.split(" ").filter((p) => p.length > 0);
                    const condiciones = palabras.map((palabra) => `ar.ar_descripcion LIKE '%${palabra}%' OR ar.ar_cod_interno LIKE '%${palabra}%' OR al.al_lote LIKE '%${palabra}%' OR ar.ar_codbarra LIKE '%${palabra}%'`);
                    where += ` AND (${condiciones.join(" AND ")})`;
                }
                if (deposito)
                ...
                if (moneda)
                    where += ` AND ar.ar_moneda IN (${moneda})`;
                if (unidadMedida)
    ...
    `;
      ...
      SELECT DISTINCT
        ...
        (case when ${moneda || 1} = 1 then 'Gs' when ${moneda || 1} = 2 then 'USD' when ${moneda || 1} = 3 then 'BRL' when ${moneda || 1} = 4 then 'ARS' else 'Gs' end) as moneda
      ${baseQuery}
      ${where}
      ORDER BY ar.ar_descripcion
      LIMIT ${limite} OFFSET ${offset}
    `;
                const [resultados, [totalRegistros]] = yield Promise.all([
                    db.sql(query),
                    db.sql(countQuery),
                ]);
    ...
    function todos(string_busqueda, id_deposito, stock) {
        ...
        if (string_busqueda) {
            ...
            const palabras = string_busqueda
                .split(" ")
                .filter((p) => p.length > 0)
                .map((palabra) => palabra.startsWith("0") ? palabra.slice(1) : palabra);
    ...
    function todosDirecta(busqueda) {
        ...
        if (busqueda && busqueda.length > 0) {
            const palabras = busqueda.split(" ").filter((p) => p.length > 0);
            const condiciones = palabras.map((palabra) => `(a.ar_descripcion LIKE '%${palabra}%' OR a.ar_codbarra LIKE '%${palabra}%')  OR a.ar_codigo LIKE '%${palabra}%'`);
            where += ` AND (${condiciones.join(" AND ")})`;
        }
        const query = `
      SELECT
    ...
    function resumen_comprasventas(fecha_desde, fecha_hasta, depositos, articulos, marcas, categorias, subcategorias, proveedores, moneda, tipo_valorizacion, talles, colores) {
        ...
        if (depositos.length > 0)
            where += ` AND de.dep_codigo IN (${depositos})`;
        if (articulos.length > 0)
            where += ` AND ar.ar_codigo IN (${articulos})`;
        if (marcas.length > 0)
            where += ` AND ar.ar_marca IN (${marcas})`;
        ...
        if (talles.length > 0)
            where += ` AND al.al_talle IN (${talles})`;
        if (colores.length > 0)
            where += ` AND al.al_color IN (${colores})`;
        ...
        let innerjoinprov = "LEFT JOIN articulos_proveedores ap ON ap.arprove_articulo = ar.ar_codigo LEFT JOIN proveedores p ON ap.arprove_prove = p.pro_codigo";
        let query = `
            SELECT
                al.al_codigo AS cod,
    ...
    function informe_stock(depositos, articulos, ubicaciones, sububicaciones, categorias, subcategorias, marcas, presentaciones, proveedores, lineas, bloques, moneda, est_stock, tipo_valorizacion, talles, colores) {
        return __awaiter(this, void 0, void 0, function* () {
            let varcampocant = `SUM(al.al_cantidad)`; //Es una variable porque puede querer sacar de histórico de stock - OPCIÓN AÚN NO IMPLEMENTADA4
            ////////////////////////////////////////////////////////Precios de compra, venta y recargo, dependiendo de la moneda
            let campocompra = "";
            ...
            if (proveedores.length > 0)
                where += ` AND p.pro_codigo IN (${proveedores})`;
            if (lineas.length > 0)
                where += ` AND ar.ar_linea IN (${lineas})`;
            if (bloques.length > 0)
                where += ` AND ar.ar_bloque IN (${bloques})`;
            if (talles.length > 0)
                where += ` AND al.al_talle IN (${talles})`;
            if (colores.length > 0)
                where += ` AND al.al_color IN (${colores})`;
    ...
    function uno(ar_codigo, id_deposito, lote) {
        const campos = " a.*, al.al_codigo, al.al_lote, al.al_cantidad, Date_Format(al.al_vencimiento, '%Y-%m-%d') AS venc, um.um_cod_set";
    ...
    function insertarArticulo(item) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const result = yield db.sql(`INSERT INTO articulos (
    ...
    function actualizarArticulo(item) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (!item.ar_codigo) {
                ...
                const setCampos = Object.entries(camposActualizar)
                    ...
                    .join(', ');
                const result = yield db.sql(`UPDATE articulos 
         SET ${setCampos} 
    ...
    function traerTodosLosArticulos() {
        return __awaiter(this, arguments, void 0, function* (pagina = 1, limite = 20, query = {}) {
            try {
    ...
    `;
                ...
                const articulos = yield db.sql(sqlQuery);
                return articulos;
            }
            catch (error) {
    ...
    function insertarInventario(consulta) {
        return __awaiter(this, void 0, void 0, function* () {
            const inventario = consulta.inventario;
            console.log("Datos de inventario recibidos:", inventario);
            const inventarioQuery = `
        INSERT INTO inventarios (fecha, hora, operador, sucursal, deposito, tipo, estado, in_obs, nro_inventario) 
        VALUES ('${inventario.fecha}', '${inventario.hora}', ${inventario.operador}, ${inventario.sucursal}, 
                ${inventario.deposito}, ${inventario.tipo}, ${inventario.estado}, '${inventario.in_obs}', ${!inventario.nro_inventario ? 1 : inventario.nro_inventario})`;
            yield db.sql(inventarioQuery);
        });
    }
    function insertarItemInventario(consulta) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                yield db.sql("START TRANSACTION");
                const ultimoInventario = yield db.sql(`
        ...
        `);
          ...
          VALUES (${detalle.idArticulo}, ${detalle.cantidad}, ${detalle.costo}, 
                  ${detalle.inventario}, ${detalle.stock_actual}, ${detalle.stock_dif})`;
                    const detalleResult = yield db.sql(detalleQuery);
              ...
              `;
                            const loteResult = yield db.sql(checkLoteQuery);
                            let loteId;
                ...
                INSERT INTO articulos_lotes (
                  ...
                  '', ${detalle.color || 0}, ${detalle.talle || 0}, ''
                                ...
                                const newLoteResult = yield db.sql(insertLoteQuery);
    ...
    function insertarItemInventarioConVencimiento(consulta) {
        return __awaiter(this, void 0, void 0, function* () {
            const { inventario, inventario_items } = consulta;
            try {
                if (!inventario ||
        ...
        `;
                ...
                yield db.sql(updateArticuloQuery);
                yield db.sql("COMMIT");
            }
            catch (err) {
                console.error("Error al insertar inventario:", err);
    ...
    function insertar_reconteo(datos) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
      ...
      `;
                yield db.sql(updateQuery);
                const inventarioQuery = `
      SELECT inventario
      FROM inventarios_items
      WHERE id = ${id_inventario_item}
      `;
                const inventario = yield db.sql(inventarioQuery);
    ...
    function insertarItemConteoScanner(datos) {
        return __awaiter(this, void 0, void 0, function* () {
      ...
      `;
                const inventario_id_result = yield db.sql(inventario_id_query);
      ...
      `;
                const inventario_item_result = yield db.sql(inventario_item_query);
      ...
      `;
                yield db.sql(inventario_item_vencimiento_query);
            }
            catch (error) {
    ...
    function reporte_reconteo() {
        return __awaiter(this, arguments, void 0, function* ({ marca, deposito, categoria, proveedor, } = {}) {
            const marcaFiltro = marca ? ` AND ar.ar_marca = ${marca}` : "";
            const depositoFiltro = deposito ? ` AND al.al_deposito = ${deposito}` : "";
            const categoriaFiltro = categoria != 0 ? ` AND ca.ca_codigo = ${categoria}` : "";
    ...
    `;
            ...
            console.log("Resultados:", yield db.sql(query));
            return db.sql(query);
        });
    }
    ...
    const inventariosDisponibles = (...args_1) => __awaiter(this, [...args_1], void 0, function* (estado = 0, deposito) {
...
`;
    ...
    const anularInventarioAuxiliar = (id) => __awaiter(this, void 0, void 0, function* () {
        // Primero verificamos el estado del inventario
        const checkQuery = `SELECT estado FROM inventario_auxiliar WHERE id = ${id}`;
        const [inventario] = yield db.sql(checkQuery);
        if (!inventario) {
    ...
    const getArticulosEnPedidos = (articulo_id, id_lote) => __awaiter(this, void 0, void 0, function* () {
        const query = `
    (SELECT
