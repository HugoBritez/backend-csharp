...
function listar(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            let procedimientos = [];
            if (flag_consultas)
                consultas = yield controlador.getConsultas(habilitar_servicios, seguro_cod, prestador, responsable, marcados, plan, buscar, dias_transcurridos);
            if (flag_estudios)
                estudios = yield controlador.getEstudios(habilitar_servicios, seguro_cod, prestador, responsable, marcados, plan, buscar, dias_transcurridos);
            if (flag_laboratorios)
                laboratorios = yield controlador.getLaboratorios(habilitar_servicios, seguro_cod, prestador, responsable, marcados, plan, buscar, dias_transcurridos);
            if (flag_imagenes)
                imagenes = yield controlador.getImagenes(habilitar_servicios, seguro_cod, prestador, responsable, marcados, plan, buscar, dias_transcurridos);
            if (flag_sanatoriales)
                sanatoriales = yield controlador.getSanatoriales(habilitar_servicios, seguro_cod, prestador, responsable, marcados, plan, buscar, dias_transcurridos);
            if (flag_procedimientos)
                procedimientos = yield controlador.getUrgencias(habilitar_servicios, seguro_cod, prestador, responsable, marcados, plan, buscar, dias_transcurridos);
            ...
            for (i in items) {
                ...
                if (items[i].habilitar === 0 || items[i].utilizar === 0)
                    items[i].estado = 0; //Gris
                ...
                const titularBuscar = yield controlador.getTitular(paciente); //Si el paciente es el titular, va a traer lo mismo
                let titular = 0;
                ...
                const pacientes = yield controlador.getAdherentes(titular); //Trae adherentes
                if (pacientes.length > 0) {
                    for (p in pacientes) {
                        pacientes_str += "," + pacientes[p].adhe;
                    }
                }
                const visaciones_realizadas = yield controlador.visacionesRealizadas(items[i].codigo, items[i].tiposer, pacientes_str);
                let suma_cantidad = 0;
...
function cobertura(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const request = req.body;
            const seguro_cod = request.seguro;
            ...
            const dias_transcurridos = Math.floor(((f_visacion_utc - f_contrato_utc) + adelanto_carencia) / _MS_POR_DIA);
            const inicio_visacion = new Date(f_visacion.getFullYear(), f_visacion.getMonth(), 1);
            let items = yield controlador.getCobertura(seguro_cod, prestador, plan, especialidad, dias_transcurridos);
            // Estado-Color de fila
            for (i in items) {
                let cantIlimitada = false;
                items[i].estado = 2; //0 gris, 1 azul, 2 verde, 3 rojo, 4 amarillo, 5 lila
...
function procedimientos(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.getProcedimientos();
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
