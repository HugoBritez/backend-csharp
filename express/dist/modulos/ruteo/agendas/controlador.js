...
module.exports = function (dbInyectada) {
    ...
    function todos(fecha_desde, fecha_hasta, cliente, vendedor, visitado, estado, planificacion, notas, orden) {
        console.log('vendedor', vendedor);
        let where = "1=1 ";
        let having = "";
        let order = "";
        if (fecha_desde != "")
            where += ` AND a_fecha >= '${fecha_desde}'`;
        if (fecha_hasta != "")
            where += ` AND a_fecha <= '${fecha_hasta}'`;
        if (cliente > 0 || cliente != null)
            where += ` AND a_cliente IN  (${cliente})`;
        if (vendedor > 0 && vendedor != null)
            where += ` AND a_vendedor IN (${vendedor})`;
        if (visitado != -1)
            where += ` AND a_visitado = ${visitado}`;
        if (estado != -1)
            where += ` AND a_estado = ${estado}`;
        if (planificacion != -1)
            where += ` AND a_planificacion = ${planificacion}`;
        if (notas == 0)
            having += " HAVING cant_notas = 0";
        if (notas == 1)
            having += " HAVING cant_notas > 0";
        switch (orden) {
            case "0": //Prioridad
...
\                    LEFT JOIN localizacion l ON a.a_codigo = l.l_agenda
    ...
    function localizaciones(id) {
        const campos = " *, DATE_FORMAT(l_fecha, '%Y-%m-%d') AS fecha, IF(l_acuracia<50, 'Precisa', IF(l_acuracia BETWEEN 50 AND 100, 'Moderada', 'Imprecisa')) AS ubicacion";
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    const query = `
          INSERT IGNORE INTO ${LOCALIZACIONES} 
          SET l_agenda = ${datos.l_agenda},
              l_fecha = '${datos.l_fecha}',
              l_hora_inicio = '${datos.l_hora_inicio}',
              l_obs = '${datos.l_obs}',
              l_cliente = ${datos.l_cliente},
              l_operador = '${datos.l_operador}',
              l_longitud = '${datos.l_longitud}',
              l_latitud = '${datos.l_latitud}',
              l_acuracia = ${datos.l_acuracia},
              l_estado = 1
        `;
    ...
    function agendamientos(desde, hasta, user, mes_actual_inicio, mes_anterior_inicio) {
        return __awaiter(this, void 0, void 0, function* () {
            let where_user = "";
            if (user != 0)
                where_user = `a_operador = ${user} AND`;
            const query = `SELECT
    ...
    function contarAgendamientos(desde, hasta, vendedor) {
        return __awaiter(this, void 0, void 0, function* () {
            let where = "";
            if (vendedor != 0) {
    ...
    function tiempoPromedioVisitas(desde, hasta, vendedor) {
        return __awaiter(this, void 0, void 0, function* () {
            let where = "";
            if (vendedor != 0)
                where = `l_operador = ${vendedor} AND `;
            const query = `SELECT
    ...
    function topVendedores(desde, hasta) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `SELECT
    ...
    function obtenerDatosGrafico(desde, hasta) {
        return __awaiter(this, void 0, void 0, function* () {
            const diffInDays = (new Date(hasta) - new Date(desde)) / (1000 * 60 * 60 * 24);
            let interval;
            let dateFormat;
  ...
  `;
            ...
            const results = yield db.sql(query);
            ...
            if (interval === "THREE_DAYS") {
                ...
                for (let i = 0; i < results.length; i += 3) {
                    ...
                    const visitados = group.reduce((sum, item) => sum + parseInt(item.visitados), 0);
                    const no_visitados = group.reduce((sum, item) => sum + parseInt(item.no_visitados), 0);
                    groupedResults.push({
    ...
    function obtenerDatosGraficoPorVendedor(desde, hasta, vendedor) {
        return __awaiter(this, void 0, void 0, function* () {
            const diffInDays = (new Date(hasta) - new Date(desde)) / (1000 * 60 * 60 * 24);
            let interval;
            let dateFormat;
  ...
  `;
            ...
            if (interval === "THREE_DAYS") {
                ...
                for (let i = 0; i < results.length; i += 3) {
                    const group = results.slice(i, i + 3);
                    const visitados = group.reduce((sum, item) => sum + parseInt(item.visitados, 10), 0);
                    const no_visitados = group.reduce((sum, item) => sum + parseInt(item.no_visitados, 10), 0);
                    groupedResults.push({
                        fecha: group[0].fecha, // Fecha del primer elemento del grupo
                        visitados,
                        no_visitados,
    ...
    function ruteos(desde, hasta, user, mes_actual, mes_anterior) {
        return __awaiter(this, void 0, void 0, function* () {
            let where_user = "";
            if (user != 0)
                where_user = `l_operador = ${user} AND`;
            const query = `SELECT
    ...
    function ruteoPorVendedor(desde, hasta) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `SELECT
    ...
    function planificacionesPorMes(desde, hasta) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `SELECT
    ...
    function planificacionesPorVend(desde, hasta) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `SELECT
                    COUNT(a_codigo) AS cantidad,
                    a_vendedor,
    ...
    function consultarPedidosYDetalles(cliente) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `
    SELECT
    ...
    function consultarVentasYDetalles(cliente) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `
    ...
    function actualizarSubvisita(datos) {
        return __awaiter(this, void 0, void 0, function* () {
            // Extraer el ID y los campos a actualizar
            const { id } = datos, camposActualizar = __rest(datos, ["id"]);
    ...
    function getSubvisitas(id_agenda) {
        return __awaiter(this, void 0, void 0, function* () {
