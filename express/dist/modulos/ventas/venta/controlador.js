...
const os = require("os");
const { VentaAdapter } = require("./venta.adapter");
const { guardarAsientoAutomatico } = require("../../contabilidad/services/guardarAsiento");
module.exports = function (dbInyectada) {
    ...
    function uno(cod) {
        ...
        const campos = " *, DATE_FORMAT(ve_fecha, '%Y-%m-%d') AS fecha, DATE_FORMAT(ve_vencimiento, '%Y-%m-%d') AS vencimiento ";
    ...
    function getCabeceras(fecha_desde, fecha_hasta, sucursal, cliente, vendedor, articulo, moneda, factura, venta, estadoVenta, remisiones, listarFacturasSinCDC, page = 1, itemsPorPagina = 50) {
        ...
        if (factura && factura.length > 0) {
            ...
            if (fecha_desde)
                where += ` AND ve.ve_fecha >= '${fecha_desde}'`;
            if (fecha_hasta)
                where += ` AND ve.ve_fecha <= '${fecha_hasta}'`;
            if (sucursal)
...
${limitOffset} `;
    ...
    function modificar(codigo, credito, fecha, vencimiento, factura, timbrado, sucursal, obs) {
    ...
    function getDetallesCliente(fecha_desde, fecha_hasta, cliente, articulo_busqueda) {
    ...
    function calcularTotalesInforme(fecha_desde, fecha_hasta, hora_inicio, hora_fin, sucursales, depositos, clientes, vendedores, articulos, tipos_articulo, marcas, categorias, subcategorias, ciudades, talles, condiciones, situaciones, movimientos, secciones, moneda, tipo_valorizacion, buscar_codigo, buscar_factura, calculo_promedio, aplica_nc, orden, desglosado_iva, desglosado_factura, agrupar_fecha, totalizar_grid, bonificacion, ncfechaVentas) {
    ...
    `;
      ...
      SELECT 
        ${desglosado_iva
                    ...
                    : `IFNULL(SUM(ndec.nc_monto),0)`} as montoncs
      ...
      SELECT 
        ${desglosado_iva
           ...
           (SUM(IFNULL(dnc.denc_diez, nc_monto)) / 11)) AS DECIMAL(12,2)), 0)`
                    : `IFNULL(SUM(dnc.denc_exentas + dnc.denc_cinco + dnc.denc_diez), 0)`} as montonc
      FROM notadecredito ndec 
      ...
      SELECT 
        ${desglosado_iva
           ...
           (SUM(IFNULL(dnd.dnd_diez, dnd.dnd_monto)) / 11)) AS DECIMAL(12,2)), 0)`
                    : `IFNULL(SUM(dnd.dnd_exentas + dnd.dnd_cinco + dnd.dnd_diez), 0)`} as montond
    ...
    `;
                ...
                if (fecha_desde && fecha_hasta) {
                    filtros.push(`ve.ve_fecha BETWEEN '${fecha_desde}' AND '${fecha_hasta}'`);
                }
                if (hora_inicio !== "00:00" || hora_fin !== "23:59") {
                    filtros.push(`ve.ve_hora BETWEEN '${hora_inicio}' AND '${hora_fin}'`);
                }
                if (sucursales === null || sucursales === void 0 ? void 0 : sucursales.length)
                    filtros.push(`ve.ve_sucursal IN (${sucursales})`);
                if (depositos === null || depositos === void 0 ? void 0 : depositos.length)
                    filtros.push(`ve.ve_deposito IN (${depositos})`);
                if (clientes === null || clientes === void 0 ? void 0 : clientes.length)
                    filtros.push(`ve.ve_cliente IN (${clientes})`);
                if (vendedores === null || vendedores === void 0 ? void 0 : vendedores.length)
                    filtros.push(`ve.ve_vendedor IN (${vendedores})`);
                if (articulos === null || articulos === void 0 ? void 0 : articulos.length)
                    filtros.push(`deve.deve_articulo IN (${articulos})`);
                if (tipos_articulo === null || tipos_articulo === void 0 ? void 0 : tipos_articulo.length)
                    filtros.push(`ar.ar_servicio IN (${tipos_articulo})`);
                if (marcas === null || marcas === void 0 ? void 0 : marcas.length)
                    filtros.push(`ar.ar_marca IN (${marcas})`);
                if (categorias === null || categorias === void 0 ? void 0 : categorias.length)
                    filtros.push(`cat.ca_codigo IN (${categorias})`);
                if (subcategorias === null || subcategorias === void 0 ? void 0 : subcategorias.length)
                    filtros.push(`ar.ar_subcategoria IN (${subcategorias})`);
                if (ciudades === null || ciudades === void 0 ? void 0 : ciudades.length)
                    filtros.push(`cli.cli_ciudad IN (${ciudades})`);
                if (talles === null || talles === void 0 ? void 0 : talles.length)
                    filtros.push(`al.al_talle IN (${talles})`);
                if (condiciones === null || condiciones === void 0 ? void 0 : condiciones.length)
                    filtros.push(`ve.ve_credito IN (${condiciones})`);
                if (situaciones === null || situaciones === void 0 ? void 0 : situaciones.length)
                    filtros.push(`ve.ve_estado IN (${situaciones})`);
                if (secciones === null || secciones === void 0 ? void 0 : secciones.length)
                    filtros.push(`ar.ar_seccion IN (${secciones})`);
                if (moneda)
                    filtros.push(`ve.ve_moneda = ${moneda}`);
                if (buscar_codigo > 0)
                    filtros.push(`ve.ve_codigo = ${buscar_codigo}`);
                if (buscar_factura)
                ...
                if (filtros.length) {
                    ...
                    if (queryBonificaciones)
                        queryBonificaciones += whereClause;
                }
                ...
                else if (desglosado_factura && (clientes === null || clientes === void 0 ? void 0 : clientes.length)) {
                    orderBy =
                ...
                const [totalesVenta, ncSinItems, notasCredito, notasDevolucion, bonificaciones,] = yield Promise.all([
                    db.sql(queryVentas),
                ...
                const totalNC = Number(((_a = ncSinItems[0]) === null || _a === void 0 ? void 0 : _a.montoncs) || 0) +
                    Number(((_b = notasCredito[0]) === null || _b === void 0 ? void 0 : _b.montonc) || 0) +
                    Number(((_c = notasDevolucion[0]) === null || _c === void 0 ? void 0 : _c.montond) || 0);
                // Calcular totales finales
                const resultado = {
                    cantidad_vendida: ((_d = totalesVenta[0]) === null || _d === void 0 ? void 0 : _d.cantidad_vendida) || 0,
                    cantidad_real: ((_e = totalesVenta[0]) === null || _e === void 0 ? void 0 : _e.cantidad_real) || 0,
                    exentas: ((_f = totalesVenta[0]) === null || _f === void 0 ? void 0 : _f.exentas) || 0,
                    iva5_total: ((_g = totalesVenta[0]) === null || _g === void 0 ? void 0 : _g.iva5_total) || 0,
                    iva5_discriminado: desglosado_iva
                        ? ((_h = totalesVenta[0]) === null || _h === void 0 ? void 0 : _h.iva5_total) / 21
                        : 0,
                    iva10_total: ((_j = totalesVenta[0]) === null || _j === void 0 ? void 0 : _j.iva10_total) || 0,
                    iva10_discriminado: desglosado_iva
                        ? ((_k = totalesVenta[0]) === null || _k === void 0 ? void 0 : _k.iva10_total) / 11
                        : 0,
                    sub_total: ((_l = totalesVenta[0]) === null || _l === void 0 ? void 0 : _l.sub_total) || 0,
                    descuento_items: ((_m = totalesVenta[0]) === null || _m === void 0 ? void 0 : _m.descuento_items) || 0,
                    descuento_factura: ((_o = totalesVenta[0]) === null || _o === void 0 ? void 0 : _o.descuento_factura) || 0,
                    costo_total: ((_p = totalesVenta[0]) === null || _p === void 0 ? void 0 : _p.costo_total) || 0,
                    cantidad_bonificada: ((_q = bonificaciones[0]) === null || _q === void 0 ? void 0 : _q.cantidad_bonificada) || 0,
                    total_bonificacion: ((_r = bonificaciones[0]) === null || _r === void 0 ? void 0 : _r.total_bonificacion) || 0,
                    total_nc: totalNC,
                    nc_sin_items: Number(((_s = ncSinItems[0]) === null || _s === void 0 ? void 0 : _s.montoncs) || 0),
                    nc_con_items: Number(((_t = notasCredito[0]) === null || _t === void 0 ? void 0 : _t.montonc) || 0),
                    notas_devolucion: Number(((_u = notasDevolucion[0]) === null || _u === void 0 ? void 0 : _u.montond) || 0),
                    total_neto: 0,
                    utilidad: 0,
    ...
    function getResumen(fecha_desde, fecha_hasta, hora_inicio, hora_fin, sucursales, depositos, clientes, vendedores, articulos, tipos_articulo, marcas, categorias, subcategorias, ciudades, talles, condiciones, situaciones, movimientos, secciones, moneda, tipo_valorizacion, buscar_codigo, buscar_factura, calculo_promedio, aplica_nc, orden, desglosado_iva, desglosado_factura, agrupar_fecha, totalizar_grid, bonificacion, ncfechaVentas) {
        return __awaiter(this, void 0, void 0, function* () {
            let where = `WHERE 1=1`;
            let mordena = "";
            let mfecha = "";
            ...
            if (sucursales.length > 0)
                where += ` AND v.ve_sucursal IN (${sucursales})`;
            if (depositos.length > 0)
                where += ` AND v.ve_deposito IN (${depositos})`;
            if (clientes.length > 0)
                where += ` AND v.ve_cliente IN (${clientes})`;
            if (vendedores.length > 0)
                where += ` AND d.deve_vendedor IN (${vendedores})`;
            if (articulos.length > 0)
                where += ` AND d.deve_articulo IN (${articulos})`;
            if (tipos_articulo.length > 0)
                where += ` AND a.ar_servicio IN (${tipos_articulo})`;
            if (marcas.length > 0)
                where += ` AND a.ar_marca IN (${marcas})`;
            if (categorias.length > 0)
                where += ` AND s.sc_categoria IN (${categorias})`;
            if (subcategorias.length > 0)
                where += ` AND a.ar_subcategoria IN (${categorias})`;
            if (ciudades.length > 0)
                where += ` AND c.cli_ciudad IN (${ciudades})`;
            if (talles.length > 0)
                where += ` AND (k.al_talle IN (${talles}))`;
            if (condiciones.length > 0)
                where += ` AND v.ve_credito IN (${condiciones})`;
            if (situaciones.length > 0)
                where += ` AND v.ve_estado IN (${situaciones})`;
            if (secciones.length > 0)
                where += ` AND a.ar_seccion IN (${secciones})`;
            if (moneda != 0)
                where += ` AND v.ve_moneda = ${moneda}`;
            if (buscar_codigo > 0)
                where += ` AND v.ve_codigo = ${buscar_codigo}`;
            if (buscar_factura > 0)
            ...
            if (vendedores.length > 0)
                mordena = " b.op_nombre, a.ar_descripcion ";
            if (marcas.length > 0)
                mordena = " m.ma_descripcion, a.ar_descripcion ";
            if (categorias.length > 0)
                mordena = " g.ca_descripcion, a.ar_descripcion ";
            if (subcategorias.length > 0)
                mordena = " s.sc_descripcion, a.ar_descripcion ";
            if (depositos.length > 0)
                mordena = " i.dep_descripcion, a.ar_descripcion ";
            if (ciudades.length > 0)
                mordena = " u.ciu_descripcion, a.ar_descripcion ";
...
`;
    ...
    function agregarCabecera(datos) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return db.agregar(CABECERA_VENTA, datos, primary_key_value, primary_key_name);
        });
    }
    function agregarDetalle(datos) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return db.agregar(DETALLES_VENTA, datos, primary_key_value, primary_key_name);
        });
    }
    //CUOTAS
    function agregarCuotas(datos) {
        return __awaiter(this, void 0, void 0, function* () {
            const primary_key_value = datos.dvc_codigo;
            const primary_key_name = "dvc_codigo";
            return db.agregar(DETALLE_VENTAS_CUOTA, datos, primary_key_value, primary_key_name);
        });
    }
    //LOTE Y STOCK
    function agregarDetalleLote(detalle, lote) {
        return __awaiter(this, void 0, void 0, function* () {
            return db.sql(`INSERT INTO ${LOTE} (id_detalle_venta, lote, loteid) VALUES (${detalle}, '', ${lote})`);
        });
    }
    function actualizarStock(cantidad, ar_codigo, al_codigo) {
        return __awaiter(this, void 0, void 0, function* () {
            return db.sql(`CALL actualizarStockVentalote(${cantidad},  ${ar_codigo}, ${al_codigo})`);
        });
    }
    //ASIENTOS CONTABLES
    function getAsientoAutomatico() {
        return __awaiter(this, void 0, void 0, function* () {
            const config = yield db.sql(`SELECT con_automatico FROM config_asiento WHERE con_nroTabla = 1`);
            return config[0].con_automatico;
        });
    }
    function asientoContableVenta(cabeceraId, venta) {
        return __awaiter(this, void 0, void 0, function* () {
            if (cabeceraId === 0) {
                //Cabecera debe ser creada, es un Insert
                const config = yield db.sql(`SELECT con_cont_prov, con_concepto FROM config_asiento WHERE con_nroTabla = 1`);
                //const cotizacion = await db.sql(`SELECT co_codigo FROM cotizaciones WHERE co_fecha = '${venta.ve_fecha}' AND co_moneda = 2`);
                const ultima_cabecera = yield db.sql(`SELECT MAX(ac_numero) as num FROM asiento_contable WHERE ac_cierre_asiento = 0`);
                let numero = 0;
                ...
                const nueva_cabecera = yield db.agregar(CABECERA_ASIENTO, cabecera, cabecera_value, cabecera_name);
                let primero = {
                    dac_codigo: 0,
                ...
                yield db.sql(`INSERT INTO ${DETALLES_ASIENTO} (dac_asiento, dac_plan, dac_debe, dac_haber, dac_concepto)
                ...
                db.sql(`UPDATE ${CABECERA_ASIENTO} SET ac_totalhaber = ${venta.total_haber} WHERE ac_codigo = ${cabeceraId}`);
            }
        });
    }
    function asientoContableCosto(cabeceraId, venta) {
        return __awaiter(this, void 0, void 0, function* () {
            if (cabeceraId === 0) {
                //Cabecera debe ser creada, es un Insert
                const config = yield db.sql(`SELECT con_gravada, con_concepto FROM config_asiento WHERE con_nroTabla = 6`);
                //const cotizacion = await db.sql(`SELECT co_codigo FROM cotizaciones WHERE co_fecha = '${venta.ve_fecha}' AND co_moneda = 2`);
                const ultima_cabecera = yield db.sql(`SELECT MAX(ac_numero) as num FROM asiento_contable WHERE ac_cierre_asiento = 0`);
                let numero = 0;
                if (ultima_cabecera[0]) {
                ...
                db.sql(`UPDATE ${CABECERA_ASIENTO} SET ${debe_str}${haber_str} WHERE ac_codigo = ${cabeceraId}`);
            }
        });
    }
    function agregarDetalleAsiento(tabla, asiento, tipo, porcentaje, debe, haber, venta) {
        return __awaiter(this, void 0, void 0, function* () {
            //Tipo: 0=Imponible(Gravadas), 1=IVA
            const config = yield db.sql(`SELECT * FROM config_asiento WHERE con_nroTabla = ${tabla}`);
            let plan = 0;
            if (tipo === 0) {
                if (porcentaje === 10) {
            ...
            return db.sql(`INSERT INTO ${DETALLES_ASIENTO} (dac_asiento, dac_plan, dac_debe, dac_haber, dac_concepto)
                        VALUES (${asiento}, ${plan}, ${debe}, ${haber}, '${config[0].con_concepto + " " + venta}')`);
        });
    }
    function actualizarMovimientoParcial(movimientoId, itemsVendidos) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
        ...
        `;
                const detallesActuales = yield db.sql(detallesActualesQuery);
                ...
                for (const item of itemsVendidos) {
                    ...
                    const detalleActual = detallesActuales.find((detalle) => detalle.dp_articulo === item.deve_articulo &&
                    ...
                    if (detalleActual) {
                        ...
                        yield db.sql(`INSERT INTO detalle_faltante (d_detalle_pedido, d_cantidad, d_situacion)
                     VALUES (${detalleActual.dp_codigo}, ${item.deve_cantidad}, 0)`);
                        ...
                        yield db.sql(`UPDATE detalle_pedido 
                     SET dp_cantidad = ${nuevaCantidad}
                     WHERE dp_codigo = ${detalleActual.dp_codigo}`);
                        console.log(`Actualizado detalle ${detalleActual.dp_codigo}: cantidad anterior ${detalleActual.dp_cantidad}, nueva cantidad ${nuevaCantidad}`);
                        console.log("*************************************************");
                    }
                    else {
                        console.log(`No se encontró detalle para el artículo ${item.deve_articulo} con lote ${item.loteid}`);
                    }
                }
    ...
    function agregarVentaDesdeDTO(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                console.log("Iniciando agregarVentaDesdeDTO");
                console.log("Venta recibida:", req.body.venta);
                console.log("Detalles recibidos:", req.body.detalle_ventas);
                const ventaAdaptada = VentaAdapter.adaptarVentaDTO(req.body.venta);
                console.log("Venta adaptada:", ventaAdaptada);
                const detallesAdaptados = req.body.detalle_ventas.map(VentaAdapter.adaptarDetalleVentaDTO);
                console.log("Detalles adaptados:", detallesAdaptados);
                const resultado = yield agregarVentaNuevo(ventaAdaptada, detallesAdaptados);
                res.status(201).json({
                    success: true,
                    message: "Venta (DTO) agregada con éxito",
                    body: resultado.ventaId
                });
            }
            catch (error) {
                console.error("Error al agregar venta desde DTO:", error);
                res.status(500).json({
                    success: false,
                    message: "Error al agregar venta desde DTO",
                    error: error.message
                });
            }
        });
    }
    function agregarVentaNuevo(venta, detalle_ventas) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (venta.pedido != null && venta.pedido != 0) {
                    try {
                        yield db.sql(`
        UPDATE pedidos
        SET p_estado = 2
      ...
      `);
                        yield actualizarMovimientoParcial(venta.pedido, detalle_ventas);
                    }
                    catch (error) {
                        console.error("Error al actualizar estado del pedido", error);
                    }
                }
                if (venta.presupuesto != null && venta.presupuesto != 0) {
                    yield db.sql(`
          UPDATE presupuesto
        ...
        ON DUPLICATE KEY UPDATE
          ...
          ve_vencimiento = '${venta.credito === 0 ? "0001-01-01" : venta.vencimiento}',
          ve_metodo = ${venta.metodo || 1}
        `);
                const ventaId = ventaQuery.insertId;
                for (const item of detalle_ventas) {
                    const detalleVentaQuery = yield db.sql(`
          ...
          `);
           ...
           `);
                    ...
                    const cantidadItemsInicial = yield db.sql(`
          ...
          `);
            ...
            `);
                        ...
                        yield db.sql(`CALL actualizarStockVentalote(${item.deve_cantidad}, ${item.deve_codigo}, ${item.loteid})`);
                    }
                }
                return {
                    status: "success",
    ...
    function agregarVenta(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                // Insert venta
                const ventaResult = yield db.agregar(CABECERA_VENTA, {
                ...
                for (const item of detalle_ventas) {
                    yield db.agregar(DETALLES_VENTA, {
                        deve_venta: ventaId,
    ...
    function ActualizarDescripcionArticulo(articulo) {
        return __awaiter(this, void 0, void 0, function* () {
            return db.sql(`INSERT INTO detalle_articulos_editado (a_detalle_venta, a_descripcion) VALUES (${articulo.deve_codigo}, '${articulo.ar_descripcion}')`);
        });
    }
    function traerIdDetalleVentas(deve_venta) {
        return __awaiter(this, void 0, void 0, function* () {
            return db.sql(`SELECT deve_codigo as detalle_venta_id,
    ...
    function idUltimaVenta() {
        return __awaiter(this, void 0, void 0, function* () {
            return db.sql("SELECT MAX(ve_codigo) as id FROM ventas");
        });
    }
    function anularVenta(ventaId, userId, clientIp, metodo, obs) {
        return __awaiter(this, void 0, void 0, function* () {
            let ventaServicioQuery = yield db.sql(`
    ...
    `);
            ...
            try {
                const existeRetencion = yield db.sql(`
      SELECT rt.r_codigo
      FROM retenciones rt
     ...
     `);
                if (existeRetencion && existeRetencion.length > 0) {
                    throw new Error("No se puede anular la venta porque tiene retenciones asociadas");
                }
                const existeCobro = yield db.sql(`
      ...
      `);
        ...
        `);
                    ...
                    const facturasRelacionadas = yield db.sql(`
          SELECT v_factura 
          FROM factura_venta 
        ...
        `);
          ...
          `);
                    ...
                    const codigoAsientoAnular = yield db.sql(`
          ...
          `);
            ...
            `;
                            yield db.sql(queryAnulacion);
                        }
                    }
                    const existeTablaServicios = yield db.sql(`
          ...
          `);
            ...
            `;
                    ...
                    const existeTablaFacturaEscribania = yield db.sql(`
              SHOW TABLES LIKE 'venta_factura_escribania'
            `);
                    if (existeTablaFacturaEscribania &&
            ...
            `;
                    ...
                    const existeTablaVentaOrden = yield db.sql(`
      ...
      `);
          ...
          `);
                ...
                `);
                        ...
                        const existeDetalleEnsamble = yield db.sql(`
              SELECT
                ae.ae_cantidad,
                ae.ae_articulo,
                ar.ar_referenciaid
    ...
    function cobrarVenta(ventaId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return db.actualizar(CABECERA_VENTA, data, primary_key_value, primary_key_name);
        });
    }
    // controlador.js
    function graficoVentas(tipo) {
        return __awaiter(this, void 0, void 0, function* () {
            let query;
            switch (tipo) {
                case "hoy":
    ...
    function totalVentasPorPeriodo(tipo) {
        return __awaiter(this, void 0, void 0, function* () {
            let query;
            switch (tipo) {
                case "hoy":
    ...
    function cantidadVentasPorPeriodo(tipo) {
        return __awaiter(this, void 0, void 0, function* () {
            let query;
            switch (tipo) {
                case "hoy":
    ...
    function cantidadArticulosVendidosPorPeriodo(tipo) {
        return __awaiter(this, void 0, void 0, function* () {
            let query;
            switch (tipo) {
                case "hoy":
    ...
    function getVentaParaImpresion(ventaId) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `
      SELECT
        ve.ve_codigo as codigo,
    ...
    function obtenerVentaParaEdicion(ventaId) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `
      SELECT
      ve.ve_codigo as id,
    ...
    const actualizarCdc = (codigo, cdc, qr) => __awaiter(this, void 0, void 0, function* () {
        const query = `
      UPDATE ventas
    ...
    const getVentasParaAgenda = (vendedor, cliente, busqueda) => __awaiter(this, void 0, void 0, function* () {
        let where = "";
        if (busqueda) {
            const palabras = busqueda.split(" ").filter((p) => p.length > 0);
            const condiciones = palabras.map((palabra) => `(ar.ar_descripcion LIKE '%${palabra}%' OR ar.ar_codbarra = '${palabra}' OR al.al_lote = '${palabra}' OR al.al_codigo = '${palabra}' )`);
