...
module.exports = function (dbInyectada) {
    ...
    function agregarPresupuesto(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { presupuesto, detalle_presupuesto } = req.body;
            try {
                // Insert venta
                const presupuestoResult = yield db.agregar(CABECERA, {
    ...
    function getCabeceras(fecha_desde, fecha_hasta, sucursal, cliente, vendedor, articulo, moneda, estado, busqueda) {
        let where = "1 = 1 ";
        let limit = "";
        if (busqueda) {
            ...
            if (fecha_desde)
                where += ` AND pre.pre_fecha >= '${fecha_desde}'`;
            if (fecha_hasta)
                where += ` AND pre.pre_fecha <= '${fecha_hasta}'`;
            if (sucursal)
                where += ` AND pre.pre_sucursal = '${sucursal}'`;
            if (cliente)
                where += ` AND pre.pre_cliente = '${cliente}'`;
            if (vendedor)
                where += ` AND pre.pre_vendedor = '${vendedor}'`;
            if (articulo)
                where += ` AND pre.pre_codigo IN (SELECT z.depre_presupuesto FROM detalle_presupuesto z WHERE depre_articulo = ${articulo})`;
            if (moneda)
                where += ` AND pre.pre_moneda = ${moneda}`;
            if (estado == 1)
                where += ` AND pre.pre_confirmado = 1`;
            if (estado == 0)
                where += ` AND pre.pre_confirmado = 0`;
        }
        if (busqueda)
            limit = ` LIMIT 10`;
        let query = `SELECT
    ...
    function actualizarPresupuestoParcial(presupuestoId, itemsEliminados) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const detallesActuales = yield getDetalles(presupuestoId);
                ...
                for (const detalleActual of detallesActuales) {
                    ...
                    if (itemEliminado) {
                        ...
                        console.log("Comparando cantidades para el item NO vendido completamente:", { cantidadActual, cantidadEliminada });
                        if (cantidadActual !== cantidadEliminada) {
                            const nuevaCantidad = cantidadActual - cantidadEliminada;
                            console.log("Actualizando cantidad del item que no se vendió completamente. Nueva cantidad:", nuevaCantidad);
                            yield db.actualizar(DETALLES, { depre_cantidad: nuevaCantidad }, detalleActual.det_codigo, "depre_codigo");
                            console.log("Cantidad actualizada en la base de datos para el item:", detalleActual.art_codigo);
                        }
                        else {
                            console.log("Las cantidades son iguales, no se requiere actualización para el item:", detalleActual.art_codigo);
                        }
                    }
                }
                ...
                for (const detalle of detallesActuales) {
                    ...
                    if (!itemNoEliminar) {
                        console.log("Eliminando detalle de venta completamente vendido:", detalle.art_codigo);
                        yield db.actualizar(DETALLES, { depre_cantidad: 0 }, detalle.det_codigo, "depre_codigo");
                    }
                    else {
                        console.log("Este item no fue vendido completamente y no se elimina:", detalle.art_codigo);
                    }
                }
                const detallesFinales = yield getDetalles(presupuestoId);
                console.log("Detalles finales después de la eliminación:", detallesFinales);
                ...
                if (todosCero) {
                    console.log("Todos los items tienen cantidad = 0, actualizando pre_confirmado a 1");
                    yield db.actualizar("presupuesto", { pre_confirmado: 1 }, presupuestoId, "pre_codigo");
                    console.log("Campo pre_confirmado actualizado a 1 en la tabla presupuesto");
                }
    ...
    function confirmarPresupuesto(presupuestoId) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    function obtenerPresupuestosParaVenta(presupuestoId) {
        return __awaiter(this, void 0, void 0, function* () {
            const query = `
      SELECT
    ...
    function insertarPresupuesto(presupuesto, detalle_presupuesto) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
        ...
        `;
                    yield db.sql(update_presupuestoQuery);
        ...
        `;
                    yield db.sql(deleteDetallesQuery);
          ...
          `;
                        yield db.sql(detallePresupuestoQuery);
                    }
          ...
          `;
                        const observacionExistente = yield db.sql(checkObservacionQuery);
              ...
              WHERE presupuesto = ${presupuesto.pre_codigo}
            `;
                            yield db.sql(updateObservacionQuery);
                        }
                        else {
            ...
            `;
                            yield db.sql(insertObservacionQuery);
                        }
                    }
        ...
        `;
                    const presupuestoResult = yield db.sql(presupuestoQuery);
