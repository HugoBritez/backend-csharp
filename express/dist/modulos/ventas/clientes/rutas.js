...
function getClientes(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const items = yield controlador.getClientes(req.query.buscar, req.query.id, req.query.id_cliente, req.query.estado, req.query.limite);
...
function todos_por_vendedor(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const items = yield controlador.todos_por_vendedor(req.params.id);
            respuesta.success(req, res, items, 200);
        }
        catch (err) {
            next(err);
...
function uno(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (item.length > 0) {
                const saldos = yield controlador.saldo(req.params.id);
                for (s in saldos) {
                    if (saldos[s].mo_codigo === 1)
                        item[0].saldog = saldos[s].saldo;
                    if (saldos[s].mo_codigo === 2)
                        item[0].saldod = saldos[s].saldo;
                    if (saldos[s].mo_codigo === 3)
                        item[0].saldor = saldos[s].saldo;
                    if (saldos[s].mo_codigo === 4)
                        item[0].saldop = saldos[s].saldo;
                }
            }
